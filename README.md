...
[![Version](https://img.shields.io/badge/version-4.12.0-blue)](CHANGELOG.md) [![status](https://img.shields.io/badge/status-personal%20tool-informational)]() [![privacy](https://img.shields.io/badge/data-stays%20local-brightgreen)]()
...
- **SAR analysis** parses pre-rendered SAR *text* tables wherever a bundle stores them: sosreport's dedicated `sos_commands/sar/*` capture, sosreport's raw `var/log/sa/*` spool directory (checked too, but only the text files in it if any exist), supportconfig's `sar/` directory (matched by directory membership, not filename, since supportconfig names files by date/day-of-month rather than anything containing "sar" — checked regardless of detected bundle `kind`, including a bundle that's *just* the `sar/` directory on its own), and crm_report's `sysstats.txt`. Handles classic (`%user`/`%system`) and modern sysstat v10+ (`%usr`/`%sys`) column naming; `MM/DD/YYYY`, `MM/DD/YY` (2-digit year — the POSIX/"C"-locale default), and ISO `YYYY-MM-DD` header date formats; and falls back to the date encoded in the file's own name when the header date can't be parsed — both the `saYYYYMMDD`/`sarYYYYMMDD` long form and the standard stock `saDD`/`sarDD` bare day-of-month form (resolved against the bundle's capture date). A mid-collection reboot's `LINUX RESTART` marker is recognized explicitly (surfaced as its own corroborating reboot timestamp) rather than disrupting whatever table was already being parsed. It does **not** decode raw binary sar spool data (`var/log/sa/saDD`), which would require shelling out to a version-matched `sar`/`sadf` binary — a dependency this tool deliberately avoids (both for portability, and because guessing at sysstat's version-specific binary record layout risks silently producing wrong numbers); when such a file is found un-rendered, it's called out by name with guidance on how to get its data instead of being silently indistinguishable from "no performance data at all". If sysstat wasn't installed on the customer's box, or no sar data was included in the capture at all, there's simply nothing to show — the Performance tab will say so rather than erroring.
