...
[![Version](https://img.shields.io/badge/version-4.14.3-blue)](CHANGELOG.md) [![status](https://img.shields.io/badge/status-personal%20tool-informational)]() [![privacy](https://img.shields.io/badge/data-stays%20local-brightgreen)]()
...
The topbar's **"💬 Feedback"** button opens a small form for general feedback or a bug report, then hands it off to your own default mail app via a `mailto:` link addressed to `skarthikeyan@microsoft.com` — nothing is sent automatically, and no bundle data/findings/AI content is ever included, only your typed message plus the app version and current job ID (if one is loaded) for context. Deliberately not real server-side email sending, since this tool has no outbound SMTP of its own by design.
...
- **📊 Performance (SAR)** — parses sysstat's pre-rendered `sar` text tables into both a condensed text summary *and* dependency-free `<canvas>` line charts on a dedicated **Performance** sub-tab (Results → Performance), picked up from both sosreport (`sos_commands/sar/`, `var/log/sa/`) and supportconfig (`sar/<day-number>`). Every metric group sysstat can produce gets its own chart(s): CPU (overall used %, full %usr/%sys/%iowait/%steal/%nice breakdown, and a per-core view when a `-P ALL` capture is present), memory (%used, used/cached/buffers in MB, %commit), disk I/O (transactions/sec and throughput, per-device when tracked), network (received/transmitted kB/s and packets/sec, per interface), load average (1/5/15 min, run queue, process count), swap, and paging. A **date/time range toolbar** lets you narrow every chart to an exact window instantly (client-side, no re-analysis) — "Apply range", "Full range" to reset, or "Zoom to CPU peak" to jump straight to ±30 minutes around the highest CPU sample. Every chart card has a **⛶ Expand** button opening a larger, more detailed view with the same interactive hover tooltip. Hover over any chart for a crosshair showing the exact timestamp/value at that point. Every timestamp is explicitly labeled with the analyzed **VM's own detected timezone** (from `/etc/timezone`, the captured `date` output, or `/etc/localtime`) so you never confuse "the time I'm reading this in" with "the time it happened on the customer's box" — a common source of confusion when the analyst and the customer are in different timezones. When sysstat's raw binary spool file (`var/log/sa/saDD`) holds real data that hasn't been rendered to text/XML yet, this is called out explicitly with guidance on how to get it, rather than looking identical to "no performance data captured at all".
- **💥 Crash / Coredump analysis** — correlates ABRT crash reports (`var/spool/abrt/ccpp-*`, already-human-readable backtraces ABRT captured at crash time), kdump/kexec configuration, and (v4.14.0) **deep vmcore binary analysis**: reads the crash dump's own header/ELF-notes directly — kernel/OS identification across any Linux flavor and version, crash timestamp, and per-CPU register state with automatic identification of the actual faulting/triggering CPU (vs. the other CPUs merely parked in the IPI-freeze handler during capture) — plus automatic discovery of kdump's own companion dmesg text log for the real panic backtrace, and resolution of the exact matching debuginfo package/install command/vmlinux path needed for full `crash`/`gdb` symbolication. All of this with **zero dependency on `crash`, `gdb`, or a matching vmlinux** — pure binary-format parsing, never shelling out to an external debugger (keeps the tool dependency-free and portable across Windows/Linux/macOS).
...
- **Symlink/hardlink handling (v4.12.0)**: on Windows (the one runtime this tool ships on), a real symlink/hardlink can't be created during extraction (no privilege by default) - Python's own `tarfile` module transparently falls back to copying the link target's real byte content in as a regular file instead, so content was already correctly readable even before v4.12.0. As of v4.12.0, every link's type/target is also tracked (`extraction_links.json`, `facts.json` → `link_summary`) so a human/AI can see *why* two paths hold identical content, instead of it looking like an unexplained coincidence. A link whose target isn't part of the bundle at all (common - most collectors don't capture `/proc`/`/dev`, or arbitrary system libraries a symlink might point at) is reported as informational context, never as "broken" - this tool has no visibility into the original source system's filesystem, so it can't and doesn't claim to know whether an unresolved target reflects a genuinely dangling link there.
- **SAR analysis** parses pre-rendered SAR data wherever a bundle stores it, in **both** the classic *text* table format and (v4.13.0) the `sadf -x` **XML** rendition sosreport often captures alongside it (content-sniffed per file, merged into the same series) — checked in sosreport's dedicated `sos_commands/sar/*` capture, sosreport's raw `var/log/sa/*` spool directory, supportconfig's `sar/` directory (matched by directory membership, not filename, since supportconfig names files by date/day-of-month rather than anything containing "sar" — checked regardless of detected bundle `kind`, including a bundle that's *just* the `sar/` directory on its own), and crm_report's `sysstats.txt`. Handles classic (`%user`/`%system`) and modern sysstat v10+ (`%usr`/`%sys`) column naming; `MM/DD/YYYY`, `MM/DD/YY` (2-digit year — the POSIX/"C"-locale default), and ISO `YYYY-MM-DD` header date formats; and falls back to the date encoded in the file's own name when the header date can't be parsed — both the `saYYYYMMDD`/`sarYYYYMMDD` long form and the standard stock `saDD`/`sarDD` bare day-of-month form (resolved against the bundle's capture date). A mid-collection reboot's `LINUX RESTART` marker is recognized explicitly (surfaced as its own corroborating reboot timestamp) rather than disrupting whatever table was already being parsed. It does **not** decode raw binary sar spool data (`var/log/sa/saDD`), which would require shelling out to a version-matched `sar`/`sadf` binary — a dependency this tool deliberately avoids (both for portability, and because guessing at sysstat's version-specific binary record layout risks silently producing wrong numbers); when such a file is found un-rendered, it's called out by name with guidance on how to get its data instead of being silently indistinguishable from "no performance data at all". If sysstat wasn't installed on the customer's box, or no sar data was included in the capture at all, there's simply nothing to show — the Performance tab will say so rather than erroring.
- **Crash/coredump analysis** correlates ABRT reports and kdump config with (v4.14.0) **direct binary analysis of the vmcore file itself** — no `gdb`/`crash`/matching-vmlinux dependency required for the metadata this extracts. Reads the vmcore's own header (makedumpfile/kdump "flattened" format, `KDUMP`/legacy `DISKDUMP` signature, or a best-effort raw-ELF fallback) to identify the exact kernel version, OS flavor (RHEL/CentOS Stream/Rocky/Alma, Fedora, Amazon Linux, SUSE, Ubuntu, Debian, or mainline — pattern-matched from the kernel-release string's vendor-specific naming convention, never hardcoded to one distro), crash timestamp, and per-CPU register state — and specifically flags the one CPU whose registers differ from the rest (the majority are parked in the kernel's own IPI/NMI freeze handler used during capture; the outlier is the CPU that was actually executing at panic time). When kdump's own companion dmesg text log is present alongside the vmcore (the common case — the same dracut/kdump machinery across RHEL/SUSE writes one automatically), its actual panic backtrace is extracted directly into the digest, and a deliberate `sysrq-trigger` test crash is called out explicitly rather than treated as an organic failure. For engineers who want to go further with `crash`/`gdb` themselves, the exact matching debuginfo package name, install command, and vmlinux path are resolved automatically from the identified OS flavor. This does **not** replace full symbolic stack-frame analysis (that still needs `crash`/`gdb` plus matching debug symbols on a Linux box) — it extracts everything genuinely available from the dump's own structure without that dependency, dramatically narrowing what a human needs to do manually afterward.
