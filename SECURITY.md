...
As of v2.2.0, whenever a **non-local** provider is selected, LDI Copilot automatically redacts the evidence digest before sending it. As of **v4.9.1**, this covers four categories:
- **Known hostnames/node names** (pulled from this analysis's own facts — e.g. the sosreport/supportconfig's captured hostname, or crm_report's detected cluster node names) are replaced with stable tokens (`HOST-1`, `HOST-2`, …). As of v4.9.1, this also matches a known hostname when it appears **fully-qualified** (e.g. `ue2op1dbsp01.corp.contoso.com`), not just standalone — earlier versions only matched the bare hostname on its own.
...
- This is a **best-effort mitigation, not a guarantee**. Hostname redaction is **allow-list based** — a hostname can't be recognized by shape alone the way an IP address can ("web01" is indistinguishable from an ordinary word), so it only catches hostnames this analysis has already independently identified elsewhere in the same bundle (its own captured hostname, or crm_report's detected node names). A peer/hostname mentioned **only** inside a large freeform verbatim block — specifically, crm_report's own "Built-in Analysis (analysis.txt, verbatim)" digest section, which is crm_report's own independent log scan and can legitimately mention identifiers (e.g. a fencing device, an NFS server) that were never one of the cluster's own node directories — will **not** be caught by hostname redaction, even though any IPv4/IPv6/email addresses in that same block still are (those three categories are detected by shape, not by an allow-list).
